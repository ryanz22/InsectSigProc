#!/usr/bin/env/python3
"""
Based on speechbrain/recipes/LibriMix/separation/train.py
...
class Separation(sb.Brain):
    ...
    # test if DP will work, TODO: DP doesn't work
        ...
        with open(save_file, "w") as results_csv:
            ...
            with tqdm(test_loader, dynamic_ncols=True) as t:
                for i, batch in enumerate(t):
                    ...
                    # # fix multiple item bug
                    # sisnr_val = -sisnr.item()
                    # if len(sisnr_val) > 1:
                    #     sisnr_val = sisnr_val[0]

                    # sisnr_i_val = -sisnr_i.item()
                    # if len(sisnr_i_val) > 1:
                    #     sisnr_i_val = sisnr_i_val[0]
                    ...
                    all_sisnrs.append(-sisnr.mean().cpu())
                    all_sisnrs_i.append(-sisnr_i.mean().cpu())
...
if __name__ == "__main__":
    ...
    if hparams["dynamic_mixing"] and not os.path.exists(hparams["base_folder_dm"]):
        print("Please, specify a valid base_folder_dm folder when using dynamic mixing")
        sys.exit(1)
